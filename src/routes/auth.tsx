...
function AuthPage() {
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    setBusy(false);
    if (error) toast.error(error.message); else navigate({ to: "/dashboard", replace: true });
  }
  ...
  async function handleSignup(e: React.FormEvent<HTMLFormElement>) {
    ...
    const parsed = signupSchema.safeParse({
      nome: fd.get("nome"), email: fd.get("email"), password: fd.get("password"),
    });
    if (!parsed.success) { toast.error(parsed.error.issues[0].message); return; }
    setBusy(true);
      ...
      <div className="hidden lg:flex flex-col justify-between p-12 bg-sidebar text-sidebar-foreground">
        ...
        <div className="space-y-4">
          ...
          <p className="text-sidebar-foreground/80 max-w-md">
            Controle do Plano de Gestão Anual, Pró-Gestão RPPS, indicadores, evidências
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignup} className="space-y-4 pt-4">
                ...
                <p className="text-xs text-muted-foreground text-center">
                  Novas contas começam com perfil de Responsável por Ação.
                </p>
              </form>
            </TabsContent>
          </Tabs>
