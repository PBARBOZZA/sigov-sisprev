...
function Areas() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => { toast.success("Área criada"); qc.invalidateQueries({ queryKey: ["areas-list"] }); setOpen(false); },
        ...
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <form onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); create.mutate({ nome: f.get("nome"), descricao: f.get("descricao") }); }} className="space-y-3">
                <div><Label>Nome</Label><Input name="nome" required /></div>
                <div><Label>Descrição</Label><Textarea name="descricao" /></div>
                <DialogFooter><Button type="submit" disabled={create.isPending}>{create.isPending && <Loader2 className="h-4 w-4 animate-spin mr-2" />}Criar</Button></DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
        )}
