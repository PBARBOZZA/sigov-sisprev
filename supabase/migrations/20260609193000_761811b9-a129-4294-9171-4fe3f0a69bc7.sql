...
-- 1. Helper function is_active_user (usada pelas policies do PGA)
...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = _user_id AND status = true
  )
$$;
...
CREATE POLICY "Active view plano anual" ON public.plano_anual FOR SELECT TO authenticated
USING (public.is_active_user(auth.uid()));
CREATE POLICY "Management manage plano anual" ON public.plano_anual FOR ALL TO authenticated
...
CREATE POLICY "Active view pga eixos" ON public.pga_eixos FOR SELECT TO authenticated
USING (public.is_active_user(auth.uid()));
CREATE POLICY "Management manage pga eixos" ON public.pga_eixos FOR ALL TO authenticated
USING (public.is_active_user(auth.uid()) AND public.is_admin_or_diretoria(auth.uid()))
WITH CHECK (public.is_active_user(auth.uid()) AND public.is_admin_or_diretoria(auth.uid()));

CREATE POLICY "Active view pga programas" ON public.pga_programas FOR SELECT TO authenticated
USING (public.is_active_user(auth.uid()));
CREATE POLICY "Management manage pga programas" ON public.pga_programas FOR ALL TO authenticated
USING (public.is_active_user(auth.uid()) AND public.is_admin_or_diretoria(auth.uid()))
WITH CHECK (public.is_active_user(auth.uid()) AND public.is_admin_or_diretoria(auth.uid()));
...
CREATE TRIGGER trg_plano_anual_updated BEFORE UPDATE ON public.plano_anual
FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE TRIGGER trg_pga_eixos_updated BEFORE UPDATE ON public.pga_eixos
FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE TRIGGER trg_pga_programas_updated BEFORE UPDATE ON public.pga_programas
FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- 3. Re-FK acoes.responsavel_id -> profiles (keep same constraint name so PostgREST embeds work)
ALTER TABLE public.acoes DROP CONSTRAINT IF EXISTS acoes_responsavel_id_fkey;
ALTER TABLE public.acoes
  ADD CONSTRAINT acoes_responsavel_id_fkey
  FOREIGN KEY (responsavel_id) REFERENCES public.profiles(id) ON DELETE SET NULL;

-- 4. Seed PGA 2026
INSERT INTO public.plano_anual (ano, nome, descricao, data_inicio, data_fim, status, versao, observacoes)
VALUES (2026, 'Plano Anual de Gestão 2026',
  'Plano Anual de Gestão do SISPREV para acompanhamento institucional do exercício de 2026.',
  DATE '2026-01-01', DATE '2026-12-31', 'em_execucao', '1.0',
  'Registro inicial criado para formalizar a Fase 1 da modelagem PGA.')
ON CONFLICT (ano) DO UPDATE SET nome = EXCLUDED.nome, updated_at = now();

WITH plano AS (SELECT id FROM public.plano_anual WHERE ano = 2026)
INSERT INTO public.pga_eixos (plano_anual_id, codigo, nome, descricao, ordem)
SELECT plano.id, v.codigo, v.nome, v.descricao, v.ordem FROM plano
CROSS JOIN (VALUES
...
ON CONFLICT (plano_anual_id, codigo) DO UPDATE SET nome = EXCLUDED.nome, updated_at = now();

WITH plano AS (SELECT id FROM public.plano_anual WHERE ano = 2026),
eixos AS (SELECT id, nome FROM public.pga_eixos WHERE plano_anual_id = (SELECT id FROM plano))
INSERT INTO public.pga_programas (plano_anual_id, eixo_id, codigo, nome, descricao, ordem)
...
-- 5. Mapeia ações existentes para eixos/programas pelo nome
WITH plano AS (SELECT id FROM public.plano_anual WHERE ano = 2026),
eixos AS (SELECT id, nome FROM public.pga_eixos WHERE plano_anual_id = (SELECT id FROM plano)),
programas AS (SELECT id, nome FROM public.pga_programas WHERE plano_anual_id = (SELECT id FROM plano))
UPDATE public.acoes a
SET plano_anual_id = plano.id,
    eixo_id = eixos.id,
    programa_id = programas.id
FROM plano
LEFT JOIN eixos ON true
LEFT JOIN programas ON true
WHERE eixos.nome = a.eixo_estrategico
  AND programas.nome = a.programa
  AND a.plano_anual_id IS NULL;
